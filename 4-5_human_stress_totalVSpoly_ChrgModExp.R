...
# Charging
ggplot(mergedf[(mergedf$pos=="Charged")&(!is.na(mergedf$odds)),], aes(x=cond,y=diff,fill=odds>0)) +
  geom_violin() +
  stat_summary(fun=median, geom="point", shape=23, size=2, position = position_dodge(width=0.9)) +
  stat_compare_means(method = "wilcox.test", label = "p.signif") +
  scale_fill_manual(values=c("#304d63ff", "#f2d096ff")) +
  ...
  geom_vline(xintercept=0) +
  theme_classic() +
  labs(y = "Delta Charged (poly-total)")
ggsave("plots/human_stress_totalVSpolyChrg_vs_oddsWT.pdf",width=8,height=3)
