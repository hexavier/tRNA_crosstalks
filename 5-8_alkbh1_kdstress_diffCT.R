...
# FOCUS ON 58-CHARGING
# Plot
ggplot(dataset[dataset$pair=="58-Charged",], aes(x=cond, y=odds)) + 
  facet_wrap( ~ reference, ncol=6, scales = "free") +
  geom_point(shape = 21, color="black", stroke=1, size=2, aes(fill = fisher)) +
  scale_fill_manual(values=c("white", "black")) +
  stat_summary(fun.data=mean_sdl, geom="pointrange", color="red") +
  stat_compare_means(ref.group = "minus-AsO2_control-shRNA", method = "t.test", label = "p.signif") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 30, vjust = 1, hjust=1))
ggsave("plots/differences_alkbh1_kdstress_58charging.pdf",width=15,height=70, limitsize = FALSE)
